...
class AuthService {
  ...
  async ensureToken() {
    if (this.hasValidToken()) {
      log.info("Token reutilizado do cookie (ainda válido).");
    ...
    return await tryCatch(
      () => this.#fetchToken(),
      (error) => log.error("Falha ao obter token.", error.message),
    );
  }
  ...
  clearToken() {
    ...
    log.info("Token removido (logout).");
  }
  ...
  async #fetchToken() {
    ...
    log.info("Token obtido com sucesso.", { expiresIn: `${data.expiresIn}s` });
    return data.token;
  }
